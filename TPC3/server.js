...
http.createServer(function (req, res) {
    ...
    if(req.url == '/'){
        ...
        // nao ordenado:  axios.get('http://localhost:3000/pessoas')
        axios.get('http://localhost:3000/pessoas?_sort=nome&_order=asc') //pode ser 'desc'
            .then(function(resp){
                var pessoas = resp.data
                
                console.log("Recuperei " + pessoas.length + " registos")       
                res.writeHead(200, {'Content-Type': 'text/html; charset=utf-8'})
                res.end(mypages.genListaPes(pessoas, d))
            })
            .catch(erro => {
